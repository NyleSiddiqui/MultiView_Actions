...
import numpy as np
from .baselinevivit import ViViT as Baseline
from .vivit import ViViT as gitviv
...
class ViViT(nn.Module):
    def __init__(self, image_size, patch_size, num_classes, num_actions, num_frames, dim = 192, depth = 4, heads = 3, pool = 'cls', in_channels = 3, dim_head = 64, dropout = 0.,
        ...
        self.transformer_encoder_layer = nn.TransformerEncoderLayer(dim, heads, activation='gelu', batch_first=True, dropout=0.0, norm_first=False)
        self.transformer_decoder_layer = nn.TransformerDecoderLayer(dim, heads, dropout=dropout, batch_first=True)
        
        self.spatial_transformer_encoder_glob = nn.TransformerEncoder(self.transformer_encoder_layer, depth)
        self.temporal_transformer_encoder_glob = nn.TransformerEncoder(self.transformer_encoder_layer, depth)
        
        self.transformer_encoder_sub = nn.TransformerEncoder(self.transformer_encoder_layer, depth)
        self.transformer_encoder_act = nn.TransformerEncoder(self.transformer_encoder_layer, depth)
        ...
        self.num_sub_queries = 20
        self.num_act_queries = 20
        
        self.sub_queries = nn.Parameter(torch.unsqueeze(get_orthogonal_queries(self.num_sub_queries, dim), dim=0))
        self.act_queries = nn.Parameter(torch.unsqueeze(get_orthogonal_queries(self.num_act_queries, dim), dim=0))
    ...
    def forward(self, x):
        ...
        x = self.spatial_transformer_encoder_glob(x)
        x = rearrange(x[:, 0], '(b t) ... -> b t ...', b=b)
        x = self.temporal_transformer_encoder_glob(x)
        
        #x_sub = self.transformer_encoder_sub(x)
        #x_act = self.transformer_encoder_sub(x)
        ...
        return (x_sub, x_act), (sub_queries, act_queries)
...
class VideoTransformer(nn.Module):
    def __init__(self, input_size, num_frames, num_subjects, num_actions, patch_size, hidden_dim, num_heads, num_layers):
        ...
        self.vivit_model = gitviv(self.num_frames, self.num_subjects, self.num_actions, self.input_size, self.patch_size, num_transformer_layers=num_layers, num_heads=num_heads).cuda()
        print(patch_size)
        print(input_size)
        pretrained = True
        if pretrained:
            model_kvpair = self.vivit_model.state_dict()
            ...
            self.vivit_model.load_state_dict(model_kvpair, strict=True)
    ...
    def forward(self, inputs):
        (features_sub, features_act), (subq, actq) = self.vivit_model(inputs)
        ...
        output_subjects = self.subject_head(features_sub)
        output_actions = self.action_head(features_act)
