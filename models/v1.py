...
class ViViT(nn.Module):
    def __init__(self, image_size, patch_size, num_classes, num_actions, num_frames, dim = 192, depth = 4, heads = 3, pool = 'cls', in_channels = 3, dim_head = 64, dropout = 0.,
        ...
        self.sub_queries = nn.Parameter(torch.randn(1, self.num_sub_queries, dim))
        self.act_queries = nn.Parameter(torch.randn(1, self.num_act_queries, dim))
        self.transformer_decoder_layer = nn.TransformerDecoderLayer(dim, heads, dropout=dropout, batch_first=True)
        self.tranformer_decoder = nn.TransformerDecoder(self.transformer_decoder_layer, depth)
    ...
    def forward(self, x):
        ...
        queries = torch.cat((sub_queries, act_queries), dim=1) # b x 16 x f
        out = self.tranformer_decoder(queries, x)
        x_sub, x_act = out[:, :self.num_sub_queries, :], out[:, self.num_sub_queries:, :]
